...
test "test_data.cmbp-1.dbn" {
    ...
    const file = try std.fs.cwd().openFile("test_data/test_data.cmbp-1.dbn", .{});
    defer file.close();
...
test "test_data.cmbp-1.v2.dbn.zst" {
    ...
    const file = try std.fs.cwd().openFile("test_data/test_data.cmbp-1.v2.dbn.zst", .{});
    defer file.close();
...
test "test_data.cmbp-1.v3.dbn.zst" {
    ...
    // Open the compressed test data file
    const file = try std.fs.cwd().openFile("test_data/test_data.cmbp-1.v3.dbn.zst", .{});
    defer file.close();
